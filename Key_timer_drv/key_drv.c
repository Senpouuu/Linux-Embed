 ...
 * @email          :  
 * @repo           :  
 * @createdOn      :  2025-3-3 20:32:15
 * @description    :  Key Timer Drv
 *------------------------------------------------------------------------**/
...
{
    printk(KERN_ALERT "key irq handler\n");
...
int key_probe(struct platform_device * pdev)
{
    ...
    if (request_irq(gpio_irq, key_irq_handler, IRQ_TYPE_EDGE_RISING, "key_irq", NULL) < 0)
    {
        printk(KERN_ALERT "Faikey to request key irq\n");
        return -EBUSY;
    } 
    
    setup_timer(&key_timer, key_timer_func, 0);
    key_timer.expires = 0;
    add_timer(&key_timer);

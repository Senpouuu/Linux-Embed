...
irqreturn_t key_irq_handler_thr(int irq, void *dev_id) 
{
    int led_stat;
    printk(KERN_ALERT "led irq handler thr\n");
...
int led_probe(struct platform_device * pdev)
{
    ...
    if (request_threaded_irq(gpio_irq, key_irq_handler,key_irq_handler_thr, IRQF_TRIGGER_FALLING, "led_irq", NULL) < 0)
    {

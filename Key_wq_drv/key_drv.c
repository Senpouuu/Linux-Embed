 ...
 * @email          :  
 * @repo           :  
 * @createdOn      :  2025-3-5 19:33:03
 * @description    :  work queue key drv 
 *------------------------------------------------------------------------**/
...
void key_wkq_func(struct work_struct *work);

DECLARE_WAIT_QUEUE_HEAD(key_wq);
DECLARE_WORK(key_wkq_st,key_wkq_func);


irqreturn_t key_irq_handler(int irq, void *dev_id) 
{
    schedule_work(&key_wkq_st) ;
    return IRQ_HANDLED;
}

void key_wkq_func(struct work_struct *work)
{
    printk(KERN_ALERT "key wkq handler\n");
